...
> ## GNU MATLAB
>
> MATLAB has only recently gained a MATLAB-like user interface. To change the
> path in any version of MATLAB, including command-line-only installations, use
...
> ## GNU MATLAB
>
> There are many small differences between MATLAB and MATLAB. One
> difference is that
> the path is *not* used by MATLAB to search for the file given in the
> the `csvread` command parameter. If you are using MATLAB you will need
